...
/**
  ...
  * @retval None
  */
static void IntToUnicode(uint32_t value, char *p_buf, uint8_t len)
{
  uint8_t idx = 0U;
...
/**
  ...
  * @retval None
  */
static void Get_SerialNum(char serial[13])
{
  ...
  */
  ...
  IntToUnicode(deviceserial0, &serial[0], 8);
  IntToUnicode(deviceserial1, &serial[8], 4);
  serial[12] = '\0';
}
/* USER CODE END 0 */
...
uint8_t *USBD_Get_String_Framework(ULONG *Length)
{
  ...
  /* Overwrite the serial number by this one */
  uint8_t USBD_SERIAL_NUMBER_BUFFER[64];
  char serial[13];
  Get_SerialNum(serial);
  snprintf(USBD_SERIAL_NUMBER_BUFFER, sizeof(USBD_SERIAL_NUMBER_BUFFER), "%s%s", USBD_SERIAL_NUMBER, serial);
  USBD_Desc_GetString((uint8_t *)USBD_SERIAL_NUMBER_BUFFER, USBD_string_framework + count, &len);
  /* USER CODE String_Framework1 */
