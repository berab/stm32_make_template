...
###################################################################################
"""
Utility functions to parse a network.c generated by the ST NPU/ATONN/NeuralART compiler
"""
...
from exceptions import CParserError
...
#   v0.3 - adding cdesc_file parsing
#   v0.4 - complete ATONN compiler options parsing
#          adding dict to return main infos
#
    ...
    if val:
        ...
        if base_.startswith('0x'):  # absolute base@
            ...
            if val1 or base_.startswith('(uintptr_t)'):
                ...
                raise CParserError(f'Unable to read the base@ from \'{base_}\'')
        return base_
    return 0
...
def _get_c_field_value(field: str, section: str,
    ...
    """Extract field value (str format)"""  # noqa: DAR101,DAR201,DAR401
...
class CompilerDesc(NamedTuple):
    ...
    virt_mempool: bool = False
    ocache_opt: bool = False
    cache_maintenance: bool = False
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'all_buffers_info': self.all_buffers_info,
            'model_file': self.model_file,
            'json_quant_file': self.json_quant_file,
            'mempool_file': self.mempool_file,
            'mdesc_file': self.mdesc_file,
            'cdesc_file': self.cdesc_file,
            'c_name': self.c_name,
            'epoch_ctrl': self.epoch_ctrl,
            'virt_mempool': self.virt_mempool,
            'ocache_opt': self.ocache_opt,
            'cache_maintenance': self.cache_maintenance,
            'version': {
                'major': self.version[0],
                'minor': self.version[1],
                'patch': self.version[2],
                'build': self.version[3],
            }
        }
...
def parse_compiler_options(lines: Union[TextIO, List[str]],
    ...
    def _get_full_path(line: str, full: bool = True) -> str:
        """Return the full path"""
        if not full:
            path_ = _get_last_item(line).replace('\\', '::').replace('/', '::')
            return path_.split('::')[-1]
        items = line.strip().split('=')
        path_ = items[-1].strip().replace('"', '')
        return str(Path(path_)) # normalize path
    ...
    all_buffers_info_, model_file_, mdesc_file_, cdesc_file_, json_quant_file_, mempool_ = False, '', '', '', '', ''
    ec_, virt_mempool_, ocache_opt_, cache_maintenance_ = False, False, False, False
    version_ = (0, 0, 0, 0)
    c_name_ = ''
    for line in lines:
        if 'GIT_DESCRIPTION' in line:
            ...
            model_file_ = _get_full_path(line)
        elif '--json-quant-file' in line:
            json_quant_file_ = _get_full_path(line)
        elif '--load-mpool-file' in line:
            mempool_ = _get_full_path(line)
        elif '--load-mdesc-file' in line:
            mdesc_file_ = _get_full_path(line)
        elif '--load-cdesc-file' in line:
            cdesc_file_ = _get_full_path(line)
        elif 'enable-epoch-controller' in line:
            ec_ = _get_last_item(line) == 'true'
        elif '--enable-virtual-mem-pools' in line:
            virt_mempool_ = _get_last_item(line) == 'true'
        elif '--Ocache-opt' in line:
            ocache_opt_ = _get_last_item(line) == 'true'
        elif '--cache-maintenance' in line:
            cache_maintenance_ = _get_last_item(line) == 'true'
        elif 'LL_ATON_VERSION_MAJOR' in line:
            line = line.replace('#if', '').replace('!=', '').replace('||', '')
            line_l = line.strip().split()
            version_ = (int(line_l[1]), int(line_l[3]), int(line_l[5]), int(line_l[7]))
    ...
    options = CompilerDesc(desc_, sha_, date_,
                           ...
                           mempool_, mdesc_file_, cdesc_file_, c_name_, ec_,
                           virt_mempool_, ocache_opt_, cache_maintenance_, version_)
...
class MemoryPool():
    ...
    def add_sub_mempool(self, cdt: 'MemoryPool', logger: Optional[logging.Logger] = None) -> bool:
        """."""
        if cdt.postfix in self._postfix.split('_'):
            self._vpool = True
            cdt.set_vid(self.pool_id)
            if not self.contains(cdt.offset, cdt.size, update_range=False, logger=logger):
                msg_ = f'Virtual mempool definition is invalid - id={cdt.pool_id} not in id={self.pool_id}'
                raise CParserError(msg_)
            if cdt.pool_id not in self._mpools:
                self._mpools.append(cdt.pool_id)
            return True
        return False

    def fix_vpool_id(self, mpools: List['MemoryPool'], logger: Optional[logging.Logger] = None) -> None:
        """Fix mempool ids for vpool support"""
        for cdt in mpools:
            if cdt.pool_id == self.pool_id:
                continue
            cdt.add_sub_mempool(self, logger=logger)

    def contains(self, base_addr: int, size: int, update_range: bool = True, is_param: bool = False,
                 logger: Optional[logging.Logger] = None) -> int:
        """Return the size of a buffer included in the mempool"""
        ...
        if is_param and use_case != 'c0' and overl_size != 0 and logger:
            msg_ = f'! ro buffer partially included in the mpool[{self.pool_id:02d}]'\
                   f':0x{self.offset:08x}:0x{self.offset + self.size - 1:08x}'\
                   f' usize={overl_size:,d}/{size:,d} buffer=0x{base_addr:08x}:0x{end_addr:08x} (uc={use_case})'
            logger.debug(msg_)
...
def parse_mempools(lines: Union[TextIO, List[str]],
    ...
    pool_descs_ = re.findall(r'\/\* index=(?P<index>[0-9]+)\s+file '
                             ...
                             s_line)
    # extern unsigned char _mem_pool_OCTOSPI1_network[]; /* [303393]; */
    mempool_c_labels_ = re.findall(r'extern unsigned char (?P<name>.*?)\[\]\;\s+\/\*\s+\[(?P<size>.*?)\]', s_line)

    if len(pool_id_sizes_) != len(pool_descs_):
        raise CParserError('Number of pools and descriptors are not aligned')
    ...
    if len(mempool_c_labels_) != nb_mempool_relative:
        raise CParserError('Number of mempool relative and c-label are not aligned')
...
class AddrDesc():
    ...
    def get_base_and_offset(self) -> Tuple[Union[str, int], int]:
...
class LLBufferDesc():
    ...
    def update_mempools(self, mempools: List[MemoryPool], logger: Optional[logging.Logger] = None):
        """."""
        for mempool in mempools:
            if self.is_relative:
                if mempool.c_label[0] == self.addr_start[0]:
                    ...
                    size_ = mempool.contains(s_addr(), e_addr() - s_addr(), is_param=self.is_param, logger=logger)
                    if size_ != self.size:
...
def parse_ll_buffers(lines: Union[TextIO, List[str]], mempools: List[MemoryPool],
    ...
    for buff_ in buffer_descriptors:
        buff_.update_mempools(mempools, logger=logger)
    ...
    for idx, line in enumerate(lines):
        ...
        if not line_descs:
            ...
            if name_ in func_descriptors:
                raise CParserError("Start/Stop function %s already parsed" % name_)
...
def parse_epoch_block_list(lines: Union[TextIO, List[str]],
    ...
    if len(start_stop_funcs):
        raise CParserError("%s func(s) not associated to a epoch descriptors" % len(start_stop_funcs))
...
class CNpuNetworkDesc():
    ...
    def __init__(self, filepath: Union[Path, str],
        ...
        self._dev_desc: DevicePropertyDesc = DevicePropertyDesc(target)
        self._to_dict: Dict[str, Any] = {}

        self._filepath = Path(filepath)
        if os.path.isdir(self._filepath):
            if f_name == 'no-name':
                f_name = 'network'
            self._filepath = Path(filepath).joinpath(f'{f_name}.c')

        if not os.path.isfile(self._filepath):
            msg_ = f'\'{self._filepath}\' is not a regular file'
            raise CParserError(msg_)
        else:
            f_name = Path(self._filepath).stem
        ...
        # complete dict to report results
        self._to_dict['compiler'] = self._options.to_dict()
        self._to_dict['io'] = self._io_to_dict()
        self._to_dict['mempools'] = self._mempools_to_dict()

        json_object = CNpuNetworkJson(self._filepath.parent,
                                      f_name=f_name,
                                      logger=logger)
            ...
            if json_object:
                if epoch.type == EpochType.BLOB:
                    ...
                    for num_id in range(epoch.epoch_num, epoch.last_epoch_num + 1):
                        ...
                        cur_perf_ = json_object.epoch_desc(cur_name_)
                        if cur_perf_['mapping'].startswith('NODE_NO_X'):
                            continue
                        elif cur_perf_['mapping'].startswith('NODE_EC') and num_id == epoch.epoch_num:
                            if cur_perf_['mapping'].split('/')[-1] != str(epoch.last_epoch_num):
                                msg_ = f"Unexpected mapping for {cur_name_}: {cur_perf_['mapping']}"
                                raise CParserError(msg_)
                            continue
                        if cur_perf_['mapping'].startswith('NODE_HW'):
                            ...
                            msg_ = f"Oh no! for {epoch.name}:{epoch.type}, {cur_perf_['mapping']} != 'NODE_HW'"
                            raise CParserError(msg_)
                    epoch.set_perf(perfs_)
                else:
                    epoch.set_perf(json_object.epoch_desc(epoch.name))
    ...
    @property
    def c_name(self) -> str:
        """Return c-name from generated file"""
        return self._options.c_name

    @property
    def f_name(self) -> str:
        """Return name w/o extension of the generated file"""
        return str(Path(self._filepath).stem)
    ...
    def _get_memories_usage(self) -> Tuple[int, int, int, int]:
        """Return memory usage (internal rw, internal const, external rw, external const)"""
        totals_ = self._to_dict['mempools']['total']
        internal_rw, internal_const = totals_['internal']['rw'], totals_['internal']['const']
        external_rw, external_const = totals_['external']['rw'], totals_['external']['const']
        return (internal_rw, internal_const, external_rw, external_const)

    def memories(self) -> Tuple[int, int, int, int]:
        """Return requested memories"""
        return self._get_memories_usage()
    ...
    def _mempools_to_dict(self) -> Dict[str, Any]:
        """Return a dict with the mempool descriptors"""
        mpool_dict: Dict[str, Any] = {'pools': [], 'total': {}}
        sizes_ = {'internal': {'rw': 0, 'const': 0}, 'external': {'rw': 0, 'const': 0}}

        for mpool in self._mpools:
            if not mpool.is_vpool:
                n_buffers_ = mpool.n_buffers()
                buffers_ = {'n_rw': n_buffers_[0][0], 'n_rw_partial': n_buffers_[0][1],
                            'n_const': n_buffers_[1][0], 'n_const_partial': n_buffers_[1][1]}
                addr_ = mpool.c_label[0] if mpool.c_label[0] else f'{mpool.offset:08x}'
                const_ = mpool.used('const')
                rw_ = mpool.used('rw')
                pool_item = {
                    'name': mpool.name,
                    'id': mpool.pool_id,
                    'attr': mpool.attrs_to_str(),
                    'used': mpool.used(),
                    'size': mpool.size,
                    'addr': addr_,
                    'data_rw': 0,
                ...
                if mpool.offset >= self._dev_desc.ext_base_address:  # 0x60000000
                    pool_item['data_rw_ext'] = rw_
                    pool_item['data_const_ext'] = const_
                    sizes_['external']['const'] += const_
                    sizes_['external']['rw'] += rw_
                else:
                    pool_item['data_rw'] = rw_
                    pool_item['data_const'] = const_
                    sizes_['internal']['const'] += const_
                    sizes_['internal']['rw'] += rw_
                mpool_dict['pools'].append(pool_item)

        mpool_dict['total'] = sizes_
    ...
    def _log_mempools(self, print_fn):
        ...
        for mpool in self._to_dict['mempools']['pools']:
            name_ = f'{mpool["name"]} ([{mpool["id"]}], {mpool["addr"]}, {mpool["attr"]})'
            const_ = _int_to_str(mpool['data_const'])
            rw_ = _int_to_str(mpool['data_rw'])
            const_ext_ = _int_to_str(mpool['data_const_ext'])
            rw_ext_ = _int_to_str(mpool['data_rw_ext'])
            buffers_ = f"{mpool['n_buffers']['n_rw']:4d}:{mpool['n_buffers']['n_rw_partial']:d} "
            buffers_ += f"{mpool['n_buffers']['n_const']:4d}:{mpool['n_buffers']['n_const_partial']:d}"
            rows_.append([name_, rw_, const_, rw_ext_, const_ext_, buffers_])

        totals_ = self._get_memories_usage()
        rows_.append(['', '', '', '', '', ''])
        rows_.append(['total', _size_int_to_desc(totals_[0]), _size_int_to_desc(totals_[1]),
                      _size_int_to_desc(totals_[2]), _size_int_to_desc(totals_[3])])

        const_ = f'{_size_int_to_desc(totals_[1] + totals_[3])}'
        rw_ = f'{_size_int_to_desc(totals_[0] + totals_[2])}'
    ...
    def _io_to_dict(self) -> Dict[str, Any]:
        """Return a dict with the IO descriptors"""
        io_dict: Dict[str, Any] = {'inputs': [], 'outputs': [], 'total': []}
        sizes_ = [0, 0]
        ...
        for buff_ in self._buffers_in:
            ...
            name_ = f'{buff_.name}{" (user)" if buff_.is_user_allocated else ""}'
            nbits_ = f'{buff_.nbits}'
            item = {
                'name': name_,
                'type': buff_.type[9:],
                'nbits': nbits_,
                ...
                'shape': shape_,
                'addr': addr_,
                'is_user_allocated': buff_.is_user_allocated
            }
            io_dict['inputs'].append(item)
        ...
        for buff_ in self._buffers_out:
            ...
            io_dict['outputs'].append(item)

        io_dict['total'] = sizes_
    ...
    def _log_io(self, print_fn):
        ...
        rows_ = []
        sizes_ = self._to_dict['io']['total']

        for item in self._to_dict['io']['inputs']:
            rows_.append([f'I: {item["name"]}',
                          f'{item["type"]} ({item["nbits"]}b)',
                          _size_int_to_desc(item['size']),
                          f'{item["shape"]}', item['addr']])

        for item in self._to_dict['io']['outputs']:
            rows_.append([f'O: {item["name"]}',
                          f'{item["type"]} ({item["nbits"]}b)',
                          _size_int_to_desc(item['size']),
                          f'{item["shape"]}', item['addr']])
    ...
    def summary(self, logger: Optional[Union[str, logging.Logger, Any]] = None, full: bool = False):
        ...
        if isinstance(logger, str):
            logger = logging.getLogger(logger)
            pr_fn = logger.info
            pr_debug_fn = logger.info if full else logger.debug
        elif callable(logger):
            pr_fn = logger
            pr_debug_fn = logger if full else None
        else:
            logger = self._logger
            pr_fn = logger.info
            pr_debug_fn = logger.info if full else logger.debug
        ...
        msg_ += f' mdesc         : {self._options.mdesc_file}\n'
        msg_ += f' cdesc         : {self._options.cdesc_file}\n'
        ...
        msg_ += f' virt mempool  : {self._options.virt_mempool}\n'
        msg_ += f' ocache opt    : {self._options.ocache_opt}\n'
        msg_ += f' cache maint.  : {self._options.cache_maintenance}\n'
    ...
    def __str__(self):
        ...
        const_ = f'({_size_int_to_desc(totals_[1])}, {_size_int_to_desc(totals_[3])})'
        ram_ = f'({_size_int_to_desc(totals_[0])}, {_size_int_to_desc(totals_[2])})'
...
def parse_c_network_file(params: Params):
    ...
    logger.info('')
    logger.info('Parsing the c-file : %s', params.input)

    c_npu_network = CNpuNetworkDesc(params.input, logger=logger)
    c_npu_network.summary(full=params.verbosity > 1)
...
def main():
    ...
    _DEFAULT_INPUT = 'st_ai_output/network.c'

    parser = argparse.ArgumentParser()

    parser.add_argument('--input', '-i', metavar='STR', type=str,
                        help='generated c-file',
    ...
    try:
        res = parse_c_network_file(params)
