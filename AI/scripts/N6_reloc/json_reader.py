...
try:
    ...
    from .exceptions import JsonParserError, BaseExceptionError
except ImportError:
    from logging_utilities import print_table, get_print_fcts
    from exceptions import JsonParserError, BaseExceptionError
...
class CNpuNetworkJson():
    ...
    def __init__(self, filepath: Union[Path, str],
        ...
        buffers_ = self._data.get('buffers', None)
        if buffers_ is None:
            msg_ = f'Invalid buffers section in JSON file (should be re-generated): {self._filepath}'
            raise JsonParserError(msg_)
        self._buffers = {buff['id']: buff for buff in buffers_}

        graphs_ = self._data.get('graphs', None)
        if graphs_ is None:
            msg_ = f'Invalid graphs section in JSON file (should be re-generated): {self._filepath}'
            raise JsonParserError(msg_)
    ...
    def _fix(self):
        """Fix possible missing data."""
        fixed_key_: str = ''
        fixed_id_: int = -1
        fixed_val_: Dict[str, Any] = {}
        for k, val in self._epochs.items():
            id_ = int(val['name'].split('_')[1].split(' ')[0])
            if 'NODE_EC' in val['mapping']:
                if val['ops'] != 0 or val['compute_cycles'] != 0 or val['max_cycles'] != 0:
                    fixed_key_ = k
                    fixed_val_ = val
                    fixed_id_ = id_
            elif fixed_val_ and 'NODE_HW' in val['mapping'] and id_ == fixed_id_:
                val['ops'] += fixed_val_['ops']
                val['compute_cycles'] += fixed_val_['compute_cycles']
                val['max_cycles'] += fixed_val_['max_cycles']
                val['mem_accesses'] = fixed_val_['mem_accesses']
                self._epochs[fixed_key_]['ops'] = 0
                self._epochs[fixed_key_]['compute_cycles'] = 0
                self._epochs[fixed_key_]['max_cycles'] = 0
                self._epochs[fixed_key_]['mem_accesses'] = {}
    ...
    def epoch_desc(self, name: str):
        """."""
        for _, val in self._epochs.items():
            if val['name'] == name:
                return val
    ...
    @property
    def n_ops(self) -> int:
        """Return accumulated number of operations."""
        return self._n_ops

    @property
    def n_cycles(self) -> Tuple[int, int]:
        """Return accumulated number of cycles (core, max)."""
        return self._n_compute_cycles, self._n_max_cycles

    @property
    def n_epochs(self) -> int:
        """Return number of epochs."""
        no_ec_ = [v for v in self._epochs.values() if not v['mapping'].startswith('NODE_EC')]
        return len(no_ec_)
    ...
    def summary(self, logger: Optional[Union[str, logging.Logger, Any]] = None):
        ...
        header_ = ['id   name', 'mapping', 'kind of operations', 'n_ops', 'n_cycles', 'r/w cycles', 'max cycles',
                   'n_ops/cycle C-M-R',
        ...
        for k, v in self._epochs.items():
            ...
            bw_ = _compute_bw_mbs(n_r + n_w, v["max_cycles"])
    ...
    def __str__(self):
        """."""
        op_per_cycles_ = self.n_ops / self.n_cycles[0]
        m_op_per_cycles_ = self.n_ops / self.n_cycles[1]
        msg_ = f'CNpuNetworkJson: n_epochs={self.n_epochs:,} n_ops={self.n_ops:,}'\
               f' cycles={self.n_cycles[0]:,} m_cycles={self.n_cycles[1]:,}'\
