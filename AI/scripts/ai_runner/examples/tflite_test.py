...
###################################################################################
"""
Example of script to compare TFLite model with generated C-model
"""
...
def stat_to_str(val):
    ...
    if val.dtype == np.float32:
        str_ = 'min={:.9f}, max={:.9f}, mean={:.1f}, std={:.1f}'.format(
            np.min(val), np.max(val), np.mean(val), np.std(val))
    else:
        str_ = 'min={}, max={}, mean={:.1f}, std={:.1f}'.format(
            np.min(val), np.max(val), np.mean(val), np.std(val))
    return str_
...
def tf_run(tf_interpreter, inputs):
    ...
    for batch in range(batch_size):
        data_input = np.expand_dims(inputs[0][batch], axis=0)
        tf_interpreter.set_tensor(input_details[0]['index'], data_input)
...
def test(args):
    ...
    print('\nCreate the interpreter for the "{}" model'.format(args.model))
    tf_interpreter = tf_create(args.model, args.verbosity)
    if tf_interpreter is None:
        print('TFLite model file should e provided, --model/-m option.')
        return 1

    print('\nCreating the ai_runner interpreter with "{}" path'.format(args.desc))
    ai_runner = AiRunner(debug=args.debug)
    if not ai_runner.connect(args.desc):
        print('No STM.AI c-model available, use the --desc/-d option to specifiy a valid path')
        return 1
