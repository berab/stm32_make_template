...
#if defined(LL_ATON_SW_FALLBACK)
...
static int helper_emit_shape_index_axis(int onnx_axis)
{
  ...
  default:
    ...
    assert(0 && "axis index not supported");
#endif
    return -1;
  }
}
...
void ll_sw_forward_conv(/* int processor, */ void *sw_info_struct)
{
  ...
  AI_TENSOR_CHAIN_OBJ_DECLARE(conv_chain, , 4, AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, TENSORS(&input_output)),
                              ...
                              AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 3, TENSORS(&conv_weights, conv_bias_ptr, NULL)),
                              AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, NULL))
...
void ll_sw_forward_resize(/* int processor, */ void *sw_info_struct)
{
  ...
  if ((ai_resize_mode)sw_info->mode == AI_RESIZE_ZEROS)
  {
    AI_LAYER_OBJ_DECLARE(resize_layer, 1, UPSAMPLE_TYPE, 0x0, NULL, upsample, forward_upsample_zeros, &resize_chain,
                         NULL, NULL, , .mode = (ai_resize_mode)AI_UPSAMPLE_ZEROS, .center = false,
                         .scales = AI_ARRAY_OBJ(&rs), .nearest_mode = AI_ROUND_PREFER_FLOOR)
...
void ll_sw_forward_activ(/* int processor, */ void *sw_info_struct)
{
  ...
  case LL_SW_SOFTMAX:
  {
    AI_LAYER_OBJ_DECLARE(nonlinearity_layer, 2, NL_TYPE, 0x0, NULL, nl, forward_sm, &activ_chain, NULL, NULL, ,
                         .nl_params = NULL, )
    nonlinearity_layer.forward(AI_LAYER_OBJ(&nonlinearity_layer));
  }
  break;
  ...
  default:
#ifdef LL_SW_ENABLE_ASSERTS
    assert(0 && "NL OPERATION NOT SUPPORTED");
#endif
    break;
  }
}
...
void ll_sw_forward_arith(/* int processor, */ void *sw_info_struct)
{
  ...
  ai_array arith_operand_array1;
  ai_array *arith_operand_array1_ptr = NULL;
  ai_array arith_operand_array2;
  ai_array *arith_operand_array2_ptr = NULL;
  if (sw_info->general.type == LL_SW_ARITHSUM)
  {
    if (sw_info->num_of_inputs >= 3)
    {
      arith_operand_array1 =
          (ai_array)AI_ARRAY_OBJ_INIT(FORMAT, sw_info->operand1.mem.start_offset, sw_info->operand1.mem.start_offset,
                                      sw_info->operand1.dim.num_elem);
      arith_operand_array1_ptr = &arith_operand_array1;
    }

    if (sw_info->num_of_inputs >= 4)
    {
      arith_operand_array2 =
          (ai_array)AI_ARRAY_OBJ_INIT(FORMAT, sw_info->operand2.mem.start_offset, sw_info->operand2.mem.start_offset,
                                      sw_info->operand2.dim.num_elem);
      arith_operand_array2_ptr = &arith_operand_array2;
    }
  }
  // tensors init
  AI_TENSOR_OBJ_DECLARE(input_output, , 0x0, 4,
  ...
  ai_tensor arith_operand1;
  ai_tensor *arith_operand1_ptr = NULL;
  if (arith_operand_array1_ptr)
  {
    arith_operand1 =
        (ai_tensor)AI_TENSOR_OBJ_INIT(0x0, 4,
                                      ...
                                      1, arith_operand_array1_ptr, NULL);
    arith_operand1_ptr = &arith_operand1;
  }
  ai_tensor arith_operand2;
  ai_tensor *arith_operand2_ptr = NULL;
  if (arith_operand_array2_ptr)
  {
    arith_operand2 =
        (ai_tensor)AI_TENSOR_OBJ_INIT(0x0, 4,
                                      ...
                                      1, arith_operand_array2_ptr, NULL);
    arith_operand2_ptr = &arith_operand2;
  }

  ai_tensor_chain arith_chain;
  if (arith_operand1_ptr && arith_operand2_ptr)
  {
    // printf("\n\n\n #################### 4 inputs\n\n\n");
    arith_chain = (ai_tensor_chain)AI_TENSOR_CHAIN_OBJ_INIT(
        AI_FLAG_NONE, 4,
        AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 4,
                                TENSORS(&input_output, &arith_operand, arith_operand1_ptr, arith_operand2_ptr)),
        AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, TENSORS(&arith_output)), AI_TENSOR_LIST_OBJ_EMPTY,
        AI_TENSOR_LIST_OBJ_EMPTY);
  }
  else if (arith_operand1_ptr)
  {
    // printf("\n\n\n #################### 3 inputs\n\n\n");
    arith_chain = (ai_tensor_chain)AI_TENSOR_CHAIN_OBJ_INIT(
        AI_FLAG_NONE, 4,
        AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 3, TENSORS(&input_output, &arith_operand, arith_operand1_ptr)),
        AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, TENSORS(&arith_output)), AI_TENSOR_LIST_OBJ_EMPTY,
        AI_TENSOR_LIST_OBJ_EMPTY);
  }
  else
  {
    // printf("\n\n\n #################### 2 inputs\n\n\n");
    arith_chain = (ai_tensor_chain)AI_TENSOR_CHAIN_OBJ_INIT(
        AI_FLAG_NONE, 4, AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 2, TENSORS(&input_output, &arith_operand)),
        AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, TENSORS(&arith_output)), AI_TENSOR_LIST_OBJ_EMPTY,
        AI_TENSOR_LIST_OBJ_EMPTY);
  }
  ...
  default:
#ifdef LL_SW_ENABLE_ASSERTS
    assert(0 && "ELEMENT WISE OPERATION NOT SUPPORTED");
#endif
    break;
  }
...
void ll_sw_forward_argmax(/* int processor, */ void *sw_info_struct)
{
  ...
  AI_LAYER_OBJ_DECLARE(argmax_layer, 1, ARGMAX_TYPE, 0x0, NULL, argmax, forward_argmax, &argmax_chain, NULL, NULL, ,
...
void ll_sw_forward_argmin(/* int processor, */ void *sw_info_struct)
{
  ...
  AI_LAYER_OBJ_DECLARE(argmin_layer, 1, ARGMIN_TYPE, 0x0, NULL, argmin, forward_argmin, &argmin_chain, NULL, NULL, ,
                       .axis = helper_emit_shape_index_axis(sw_info->axis),
                       .select_last_index = sw_info->select_last_index, )
...
void ll_sw_forward_reduce(/* int processor, */ void *sw_info_struct)
{
  ...
  else
  {
    ...
    default:
#ifdef LL_SW_ENABLE_ASSERTS
      assert(0 && "REDUCTION OPERATIONS NOT SUPPORTED");
#endif
      break;
    }
...
void ll_sw_forward_tile(/* int processor, */ void *sw_info_struct)
{
  ...
  AI_ARRAY_OBJ_DECLARE_STATIC(repeats, ai_i16, AI_ARRAY_FORMAT_S16, AI_CONST, 3, sw_info->repeats[2],
                              sw_info->repeats[3], sw_info->repeats[1])
